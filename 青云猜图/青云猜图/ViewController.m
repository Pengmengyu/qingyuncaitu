...
@property (weak, nonatomic) ViewController *weakSelf;
@end
    ...
    [_answerView.subviews enumerateObjectsUsingBlock:^(__kindof UIView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if ([obj isKindOfClass:[UIButton class]]) {
            ...
            if (![rightString isEqualToString:((UIButton *)obj).currentTitle]) {
                ...
                [_weakSelf answerBtnClick:(UIButton *)obj];
                //*stop = YES;
            }
        }
    }];
            ...
            if ([obj isKindOfClass:[UIButton class]]) {
                ...
                if ([optionBtn.currentTitle isEqualToString:rightString]) {
                    ...
                    *stop = YES;
                    ...
                    question.isHint = YES;
                    //2、减金币
                    [_weakSelf changeCoin:-1000];
                    //3、模拟点击optionBtn
                    [_weakSelf optionBtnClick:optionBtn];
                }
            }
        }];
    }
...
-(void)next{
    ...
    if (isPass) {
        ...
        UIAlertAction *action = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDestructive handler:^(UIAlertAction * _Nonnull action) {
            [_weakSelf.questions enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                QYQuestion *question = (QYQuestion *)obj;
                question.isHint = NO;
...
-(void)updateUI{
    ...
    answerView.answerBtnClick = ^(UIButton *answerBtn){
        [_weakSelf answerBtnClick:answerBtn];
    };
